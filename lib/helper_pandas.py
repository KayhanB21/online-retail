...
def null_val_summary(df: pd.DataFrame) -> None:
    ...
    null_info = null_info.append(pd.DataFrame(df.isnull().sum() / len(df.index) * 100).T.rename(index = {0: 'null values (%)'}))
    display(null_info)


def unique_col_percent(df: pd.DataFrame) -> None:
    for col in df.columns:
        print(f"{col} unique count and percentage: {len(df[col].unique())}, {len(df[col].unique()) / len(df.index) * 100:0.2f}")
